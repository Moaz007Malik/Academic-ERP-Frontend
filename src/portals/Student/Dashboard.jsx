...
export default function StudentDashboard() {
  ...
  return (
    <>
      <PageTitle title={`Welcome, ${student.firstName}`} subtitle={`${student.currentBatch?.name || ''} ${student.currentSection?.name ? `— Section ${student.currentSection.name}` : ''}`} />
      ...
      {recentResults?.length === 0 ? <p className="text-sm text-gray-500">No published results yet</p> : (
