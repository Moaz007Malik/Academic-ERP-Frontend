...
export default function StudentTicketDetail() {
  ...
  const load = () => api.get(`/student/tickets/${id}`).then((res) => setTicket(res.data.data));

  useEffect(() => { load(); }, [id]);

  const send = async (e) => {
    e.preventDefault();
    await api.post(`/student/tickets/${id}/reply`, { message: reply });
    setReply('');
    load();
  };
  ...
  return (
    <>
      <PageTitle title={ticket.subject}><Link to="/student/tickets"><Button variant="secondary">Back</Button></Link></PageTitle>
