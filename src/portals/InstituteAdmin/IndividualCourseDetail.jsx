...
export default function IndividualCourseDetail() {
  ...
  return (
    <DetailPageLayout
      ...
      tabs={[{ id: 'overview', label: 'Overview' }, { id: 'enrollments', label: 'Enrollments' }, { id: 'fees', label: 'Fees' }]}
      activeTab={tab}
      onTabChange={setTab}
    >
      {tab === 'overview' && (
      ...
      <Modal open={enrollOpen} onClose={() => setEnrollOpen(false)} title="Enroll Student" wide>
        <form onSubmit={enroll} className="space-y-3">
          ...
          {enrollForm.mode === 'existing' ? (
            <Input label="Student ID" value={enrollForm.studentId || ''} onChange={(e) => setEnrollForm({ ...enrollForm, studentId: e.target.value })} placeholder="Paste student ID from profile URL" />
          ) : (
            <>
